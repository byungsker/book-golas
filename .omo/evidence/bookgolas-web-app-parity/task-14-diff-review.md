...
- CodeRabbit review: CodeRabbit CLI 0.7.6 initially raised one major accessibility issue and one minor locale-state issue; the committed follow-up review raised 0 issues. The GitHub PR review then raised three actionable issues covering the dialog, the Recall `mode=recall` contract, and unavailable collection state. All three were verified and fixed with the existing Radix dialog, the contract-defined Recall URL, and the ordered query boundary. Follow-up unit, browser, and full regression checks pass.
- Review hardening: the follow-up local review also covered test environment restoration and modified-click behavior. The query fixture now restores environment variables in `finally`, and tab links preserve browser behavior for Ctrl/Cmd/Shift/Alt clicks while keeping client-side navigation for ordinary clicks.
