...
<ul>
<li><strong>Google Cloud Vision:</strong> 선택·촬영·자른 책 페이지 이미지와 한국어/영어 언어 힌트를 OCR 실행 시마다 TLS로 암호화하여 미국 리전 엔드포인트로 전송합니다. 제공받는 자는 Google LLC(1600 Amphitheatre Parkway, Mountain View, CA 94043, USA)이며 Google Cloud 개인정보 보호 문의 양식은 cloud.google.com/privacy에서 확인할 수 있습니다. 목적은 이미지 문자 추출입니다. 이미지는 온라인 Vision 서비스의 메모리에서 처리되고 저장되지 않으며, 요청 메타데이터는 공개된 정확한 기간 없이 보안·운영을 위해 일시적으로 기록될 수 있습니다.</li>
<li><strong>OpenAI:</strong> 기능에 필요한 책 제목·저자·장르, 독서 상태·날짜·페이지 진행률, 평점·리뷰, 메모·하이라이트·OCR 텍스트, 독서 속도·목표 달성·시도·참여 통계와 질문을 TLS로 암호화하여 전송합니다. 주된 이전 국가와 제공받는 자는 미국의 OpenAI OpCo, LLC(1455 3rd Street, San Francisco, CA 94158, USA, privacy@openai.com)입니다. 글로벌 네트워크 라우팅, 인프라, 보안 검토 또는 지원이 적용되면 OpenAI가 공개한 하위처리자가 해당 공개 국가에서 조건부로 처리할 수 있습니다(openai.com/policies/sub-processor-list). 사용자가 AI 기능을 실행할 때 전송하며, 추천 화면 진입 시 캐시가 없거나 만료되면 추천을 자동 생성하고, 동의 후 메모·하이라이트·OCR 기록 저장 시 의미 검색용 임베딩을 자동 생성할 수 있습니다. 목적은 AI 검색·요약·인사이트·추천·글쓰기 지원과 의미 검색용 임베딩 제공입니다. API 입출력은 기본적으로 모델 학습에 사용되지 않습니다. 오남용 모니터링 로그는 최대 30일(보호·법적 필요 시 연장), 프롬프트 캐시 상태는 해당 기능 적용 시 최대 24시간 보관될 수 있습니다.</li>
</ul>
<p>Google 이전을 거부하면 OCR을 사용할 수 없지만 직접 입력과 다른 독서 관리 기능은 이용할 수 있습니다. OpenAI 이전을 거부하면 OpenAI 기반 AI 기능과 의미 검색을 사용할 수 없지만 다른 독서 관리 기능은 이용할 수 있습니다. 마이페이지에서 업체별로 동의하거나 철회할 수 있습니다. 철회 성공이 서버에서 확인된 뒤 시작하는 새 요청은 차단되며 이미 시작된 요청은 완료될 수 있습니다. 고지 내용이나 처리 국가 등 중요 사항이 변경되면 새 고지 버전으로 다시 동의를 받습니다.</p>
...
<p>동의 철회 성공이 서버에서 확인되면 그 뒤 시작하는 새 요청의 전송이 중단됩니다. 이미 시작된 요청은 완료될 수 있습니다. 철회 전에 생성된 외부 처리 로그의 삭제 또는 보관은 각 업체의 정책과 법적 의무에 따릅니다. 관련 요청은 고객 지원으로 문의할 수 있습니다.</p>
...
<ul>
<li><strong>Google Cloud Vision:</strong> A selected, captured, or cropped book-page image and Korean/English language hints are sent with TLS encryption to the US regional endpoint each time OCR runs. The recipient is Google LLC, 1600 Amphitheatre Parkway, Mountain View, CA 94043, USA; the Google Cloud privacy contact form is at cloud.google.com/privacy. The purpose is image text extraction. The image is processed in memory and is not persisted by the online Vision service; request metadata may be logged temporarily for security and operations without a published exact duration.</li>
<li><strong>OpenAI:</strong> As needed, book title, author, genre, reading status and dates, page progress, rating, review, notes, highlights, OCR text, reading pace, goal achievement, attempts, engagement statistics, and the user's question are transferred with TLS encryption. The primary country and recipient are the United States and OpenAI OpCo, LLC, 1455 3rd Street, San Francisco, CA 94158, USA, privacy@openai.com. When global network routing, infrastructure, security review, or support applies, OpenAI's disclosed subprocessors may process data conditionally in their disclosed countries (openai.com/policies/sub-processor-list). Data is sent when the user runs an AI feature; recommendations are generated automatically when the recommendation screen opens and the cache is missing or expired, and embeddings may be generated automatically when a note, highlight, or OCR record is saved after consent. The purposes are AI search, summaries, insights, recommendations, writing assistance, and semantic-search embeddings. API input and output are not used for training by default. Abuse-monitoring logs may be retained up to 30 days (longer for protection or legal needs), and prompt-cache application state may be retained up to 24 hours when applicable.</li>
</ul>
<p>Refusing Google transfer disables OCR but manual entry and other reading-management features remain available. Refusing OpenAI transfer disables OpenAI-powered AI features and semantic search but other reading-management features remain available. Consent can be granted or withdrawn separately in My Page. New requests started after withdrawal is confirmed by the server are blocked; a request already underway may finish. Material disclosure changes, including processing-location changes, require consent to the new disclosure version.</p>
